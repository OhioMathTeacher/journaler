...
"""Drive the real app in a headless browser and report what it actually did.
    ...
    python3 tools/probes/run.py            # page eviction, note anchoring, canvas memory
...
def make_page(suite):
    ...
    # ⚠ A CLASSIC script tag, deliberately. The app's own big block is classic, so its
    # top-level functions are on the global object and the driver can call them by name.
